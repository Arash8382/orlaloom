...
export function getCategoryProducts(categorySlug) {
  ...
  for (const p of getPostsByCategory(categorySlug)) {
    ...
    (data.products || []).forEach((pr) => {
      if (pr && pr.name) out.push({ ...pr, guideSlug: p.slug, guideTitle: p.title });
    });
  }
  return out;
}
...
export function getAllProducts() {
  ...
  for (const p of getAllPosts()) {
    ...
    (data.products || []).forEach((pr) => {
      if (pr && pr.name) {
        out.push({
          name: pr.name, brand: pr.brand || "", price: pr.price || "",
...
export function getProductMap() {
  ...
  for (const p of getAllPosts()) {
    ...
    (data.products || []).forEach((pr) => {
      ...
      map.set(slug, { ...pr, slug, category: p.category, guides: [{ slug: p.slug, title: p.title }] });
    });
  }
  return map;
}
