...
// Freshness sort: a guide that gained new products today is as "fresh" as one
// published today, and during a publishing freeze it is the ONLY thing that is
// fresh. The homepage promises "new finds daily" — that promise is kept by
// adding products to existing guides, so recency must count `updated`.
export function getPostsByFreshness() {
  return getAllPosts()
    .slice()
    .sort((a, b) => {
      const av = a.updated || a.date;
      const bv = b.updated || b.date;
      return av < bv ? 1 : -1;
    });
}
    ...
    (data.products || []).forEach((pr, idx) => {
      ...
      if (map.has(slug)) {
        ...
        if (existing.url && pr.url && existing.url === pr.url) {
          ...
          const f2 = p.updated || p.date || "";
          if (f2 > (existing._fresh || "")) { existing._fresh = f2; existing._pos = idx; }
          return;
        }
      ...
      map.set(slug, {
        ...
        slug,
        category: p.category,
        guides: [{ slug: p.slug, title: p.title }],
        // Ordering metadata. _fresh = when the guide this product came from was
        // last touched (`updated`, falling back to `date`); _pos = its index in
        // that guide's product list. New picks are appended, so a higher _pos
        // means more recently added.
        _fresh: p.updated || p.date || "",
        _pos: idx,
        // Per-PRODUCT added date. Falls back to the guide's date so pre-existing
        // products aren't all treated as brand new. The daily product task writes
        // `added:` on each product it appends.
        _added: pr.added || p.date || "",
      });
    });
  }
  return map;
...
// Products ordered by how recently they appeared on the site: freshest guide
// first, and within a guide the most recently appended picks first.
//
// Deliberately a separate function rather than reordering getProductMap(): that
// map's INSERTION order decides which product wins the plain /shop slug when two
// share a name, so reordering it would silently change already-indexed product
// URLs. This only affects display order.
//
// The homepage grid used raw getProductMap() order, which is guide PUBLISH date.
// That froze the grid to whatever was published last and meant it could not
// change at all while publishing is paused, even as products were being added.
// Homepage product ordering. Two goals, and the old version served neither well:
//
//   1. GENUINELY NEW products must surface. Sorting by the guide's `updated` promoted
//      the guide's ENTIRE product list, so adding 2 products to a 24-product guide
//      pushed all 24 to the top and the grid filled with old items.
//   2. The grid must LOOK DIFFERENT DAY TO DAY, the way a storefront does. A pure
//      recency sort is stable, so a returning visitor saw an identical page until
//      something was published.
//
// So: products carry their own `added` date (set by the daily product task when it
// adds them). Tier 1 is products added in the last NEW_WINDOW_DAYS, newest first —
// that is the "new arrivals" row. Tier 2 is everything else, shuffled with a seed
// derived from the build date, so each deploy reshuffles the tail. The site rebuilds
// daily, so the page changes daily even on a day nothing was added.
const NEW_WINDOW_DAYS = 10;

// Deterministic per-day shuffle: same order all day, different order tomorrow.
...
export function getProductsByFreshness() {
  const all = [...getProductMap().values()];
  const today = new Date();
  const daySeed = Math.floor(today.getTime() / 86400000); // changes once per day
  const cutoff = new Date(today.getTime() - NEW_WINDOW_DAYS * 86400000)
    .toISOString()
    .slice(0, 10);

  const isNew = (p) => (p._added || "") >= cutoff;

  const fresh = all
    .filter(isNew)
    .sort((a, b) => {
      if (a._added !== b._added) return a._added < b._added ? 1 : -1;
      return (b._pos || 0) - (a._pos || 0);
    });

  const rest = all
    .filter((p) => !isNew(p))
    .sort((a, b) => seededRank(a.slug, daySeed) - seededRank(b.slug, daySeed));
