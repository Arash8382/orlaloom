      ...
      {products.length > 1 && (
        <section className="ge-compare">
          ...
          <div className="ge-card">
            ...
            {products.map((pr, i) => {
              const Row = pr.url ? "a" : "div";
              return (
                <Row
                  key={i}
                  className="ge-row"
                  {...(pr.url ? { href: pr.url, target: "_blank", rel: "nofollow sponsored noopener", "aria-label": `Shop ${pr.name}` } : {})}
                >
...
.ge-row{display:grid;grid-template-columns:minmax(0,1fr) 150px 96px 92px;align-items:center;gap:12px;padding:12px 18px;text-decoration:none;color:inherit;border-top:1px solid var(--line,#f0e6d8);transition:background .15s}
.ge-row:first-child{border-top:0}
a.ge-row:hover{background:var(--cream,#faf4ea)}
...
@media(max-width:640px){
  .ge-row{grid-template-columns:minmax(0,1fr) 78px auto;gap:10px;padding:11px 13px}
  .ge-col-best{display:none}
  .ge-name{font-size:14px}
  .ge-shop{padding:6px 11px;font-size:11.5px}
}`;
